...
//        query = "SELECT od.id, od.total_price, od.size size_vn, b.title product_name, (p.images ->> '$[0]') as product_img " +
//                "FROM orders od " +
//                "INNER JOIN book b " +
//                "ON od.book_id = b.id " +
//                "WHERE od.status = ?1 " +
...
//        query = "SELECT orders.id, orders.total_price, book.title product_name, orders.price as product_price, " +
//                "orders.receiver_name, orders.receiver_phone, orders.receiver_address, orders.status, " +
//                "product.images ->> \"$[0]\" as product_img " +
//                "FROM orders " +
//                "INNER JOIN book " +
//                "ON orders.book_id = book.id " +
